...
def run_adaptive_forecast_system(full_df):
    ...
    for months in split_months_options:
        ...
        if test_df.empty or len(train_df) < 36: # 确保有足够数据
            print(f"      警告: 数据不足，跳过此实验。")
            continue
        ...
        print(f"      训练集截止: {train_df.index.max().strftime("%Y-%m")}, 测试集: {test_df.index.min().strftime("%Y-%m")} to {test_df.index.max().strftime("%Y-%m")}")
    ...
    # --- 步骤 3: 选出“冠军”并进行最终预测 ---
    if not race_results:
        raise ValueError("所有“赛马”实验均失败，无法进行最终预测。")
    ...
    for res in race_results:
        ...
        print(f"-{res['offset']:<7}  | {res['rmse']:<6.2f} |{is_champion}")
    
    print(f"\n冠军模型: {-champion['offset']}个月的训练周期 (训练至 {champion['train_end_date'].strftime('%Y-%m')})。")
    print("现在将使用此‘冠军’配置进行最终预测...")
    ...
    print(f"      ...动态预测出未来 {steps_to_forecast} 个月的ENSO异常值为: {future_enso_values.iloc[-1]:.2f}")
    ...
    print(f"> 预测的期望值 (均值): {predicted_mean:.2f}")
    print("\n--- 模型预测概率分布 ---")
